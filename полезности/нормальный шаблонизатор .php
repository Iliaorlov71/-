<?//нормальный шаблонизатор для сео - битрикса ,обходит стандартный функционал сео , для установки - подключить файл в catalog-section и в catalog-element в компонент- эпиоге , вывод сео - в резалт-модифайре на эпилоге , для элементов- задаютсся новые свойства , для секций - польз.поля Uf_GEN...-генерация , UF_NOGEN_собств титл  , внутри - рекурсивная функция , проходится по родителям и собирает массив полей сео ген. Пользуйте наздоровье.Свойства тут посмотри.
...
if(!$arResult["PATH"][0]["LIST_PAGE_URL"]=="/catalog/"){
  ...
  $ENDMETA["TITLE"]="Аккумулятор {name} по низкой цене с доставкой по всей России | Интернет-магазин AKBMOSCOW
";
  };
  ...
  $ParentCode=$arResult["SECTION"]["ID"];
  $ENDMETA["DESCRIPTION"]="Аккумулятор {name} недорого в интернет-магазине AKBMOSCOW. Гарантия от производителя. Бесплатная доставка по Москве и области.
";
  };
  ...
  $ParentCode=$arResult["SECTION"]["ID"];
  $ENDMETA["KEYWORDS"]="аккумулятор {name} купить цена недорого интернет магазин доставка
";
  };
